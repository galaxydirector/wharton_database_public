...
def process_info_columns(df,i,columns,cf="info"):
    ...
    for column in columns:
        ...
        if not pd.isnull(value):
            row_value[':'.join((cf, column))] = str(format(abs(value),".8f")) if (column=="ret" or column=="retx") else str(value) # format(abs(value),".8f"),abs(value)
...
def write_to_hbase(pool, table_name, crsp_split, ccm_merged, step):
    ...
    with pool.connection() as connection:
        ...
        # add all column type, hard coding
...
#         for i in range(10):
        ...
        # dump data from ccm_merged from crsp 
...
#         for i in range(500):
            ...
            row_key_ccm_price, row_value_ccm_price = process_priceVol_columns(ccm_merged,i,columns=priceVol_col_name,cf="priceVol")
